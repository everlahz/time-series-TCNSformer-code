...
def model_factory(config):
    if config['Net_Type'][0] == "PPSN":
        ...
        model= TCNSFormer(config) #实例化一个对象
    return model
...
class TCNSFormer(nn.Module):
    def __init__(self, config):
        ...
        # self.local_pos_embedding = self.create_position_embedding(40, 128)
        # Create positional embeddings (you can use learned embeddings or predefined like Sinusoidal)
    ...
    def forward(self, x, ep):
       ...
       # print(f'h.shape:{h.shape}')
        ...
        self.pos = torch.arange(0, global_x.shape[1]).unsqueeze(0).to('cuda')
        self.pos_embedding = nn.Embedding(global_x.shape[1], global_x.shape[2]).to('cuda')
        
        self.local_pos_embedding=self.pos_embedding(self.pos)
        

        
        global_x = global_x + self.local_pos_embedding
